...
export default function FolderList({
  ...
  const getDueDateStatus = (dueDate) => {
    ...
    if (adjustedDiffDays <= 0)
      return { type: "overdue", text: "Tarea vencida", color: "bg-red-500" };
    if (adjustedDiffDays === 1)
      return { type: "today", text: "Vence hoy", color: "bg-orange-500" };
    if (adjustedDiffDays === 2)
      return { type: "oneDay", text: "Vence mañana", color: "bg-yellow-500" };
    if (adjustedDiffDays === 3)
      return { type: "twoDays", text: "Vence pasado mañana", color: "bg-green-500" };
    return null;
  };
                    ...
                    {folder.upcomingTasks.map((task) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {dueDateStatus && (
                            <span
                              className={`px-2 py-1 rounded text-xs font-semibold animate-pulse cursor-default shadow-md ${dueDateStatus.color} text-black`}
                            >
                              {dueDateStatus.text}
                            </span>
                          )}
